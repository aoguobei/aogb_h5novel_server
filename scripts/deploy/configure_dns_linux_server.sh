#!/bin/bash
# scripts/configure_dns_linux_server.sh
# Linux服务器版本DNS配置脚本
...
# Linux服务器DNS配置相关路径
# ========================================
DNSMASQ_CONF_PATH="/etc/dnsmasq.conf"                                      # dnsmasq配置文件路径
DNSMASQ_BACKUP_ROOT="/opt/nginx_dns_backups/dns_backup"                              # DNS配置备份目录
...
backup_dns_config() {
    ...
    sudo mkdir -p "$DNSMASQ_BACKUP_ROOT"
    
    if [ -f "$DNSMASQ_CONF_PATH" ]; then
        local dns_backup_dir="$DNSMASQ_BACKUP_ROOT/$(date +%Y%m%d_%H%M%S)"
        sudo mkdir -p "$dns_backup_dir"
        sudo cp "$DNSMASQ_CONF_PATH" "$dns_backup_dir/"
...
auto_backup_recovery() {
    ...
    if [ -f "$DNSMASQ_CONF_PATH" ]; then
        sudo cp "$DNSMASQ_CONF_PATH" "$recovery_backup_dir/"
        echo "✅ 当前配置已备份到: $recovery_backup_dir"
    fi
    
    # 恢复配置
    sudo cp "$backup_file" "$DNSMASQ_CONF_PATH"
    ...
    if validate_dns_config; then
        ...
        if restart_dns_service; then
            ...
            if [ -f "$recovery_backup_dir/dnsmasq.conf" ]; then
                sudo cp "$recovery_backup_dir/dnsmasq.conf" "$DNSMASQ_CONF_PATH"
                echo "✅ 已回滚到恢复前的配置"
            fi
            return 1
        fi
...
add_dns_config() {
    ...
    # 添加DNS配置
    sudo tee -a "$DNSMASQ_CONF_PATH" >/dev/null << EOF

# 脚本添加的本地测试域名配置 - $domain - $(date)
...
cleanup_dns_config() {
    ...
    if [ -f "$DNSMASQ_CONF_PATH" ]; then
        # 创建临时文件，直接过滤掉相关配置
        local temp_file="/tmp/dnsmasq_temp_$$"
        ...
        # 替换原文件
        sudo mv "$temp_file" "$DNSMASQ_CONF_PATH"
...
show_usage() {
    ...
    echo ""
    echo "常规DNS配置:"
    echo "  $0 \"test.example.com\" \"true\" \"create_server\""
    echo "  $0 \"localhost\" \"false\" \"add_location\""
    echo ""
    echo "清理DNS配置:"
    echo "  $0 \"test.example.com\" \"cleanup\" \"rollback\""
    echo ""
    echo "备份恢复操作:"
