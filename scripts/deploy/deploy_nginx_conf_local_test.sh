#!/bin/bash
# scripts/deploy_nginx_config_local_test.sh
# 本地测试版本的nginx配置部署脚本 - Windows环境适配
...
# 本地测试配置文件路径配置
# ========================================
# Windows环境下的nginx配置路径（请根据实际安装路径修改）
NGINX_CONF_PATH="C:/F_explorer/env/nginx-1.24.0/nginx-1.24.0/conf/nginx.conf"    # nginx配置文件路径
NGINX_BIN_PATH="C:/F_explorer/env/nginx-1.24.0/nginx-1.24.0/nginx.exe"            # nginx可执行文件路径
NGINX_LOGS_PATH="C:/F_explorer/env/nginx-1.24.0/nginx-1.24.0/logs"                # nginx日志目录路径
BACKUP_ROOT="C:/F_explorer/env/opt/backups/nginx_conf_backup"                                         # 本地备份目录

# DNS配置脚本路径
DNS_SCRIPT_PATH="$(dirname "$0")/configure_dns_local.sh"
...
echo "🚀 本地测试部署配置: $DOMAIN -> $LOCATION_PATH (端口: $PORT)"
...
add_location() {
    local start_line="$1"
    local end_line="$2"
    local temp_file="$(dirname "$NGINX_CONF_PATH")/nginx_temp_$$"
...
main() {
    ...
    # 验证nginx配置（使用绝对路径避免相对路径问题）
    if ! "$NGINX_BIN_PATH" -t -c "$NGINX_CONF_PATH" -p "$(dirname "$NGINX_BIN_PATH")" -e "$NGINX_LOGS_PATH/error.log" -g "pid $NGINX_LOGS_PATH/nginx.pid;" 2>/dev/null; then
    ...
    if ! "$NGINX_BIN_PATH" -s reload -c "$NGINX_CONF_PATH" -p "$(dirname "$NGINX_BIN_PATH")" 2>/dev/null; then
...
rollback_configs() {
    ...
    if ! "$NGINX_BIN_PATH" -s reload -c "$NGINX_CONF_PATH" -p "$(dirname "$NGINX_BIN_PATH")"; then
        echo "❌ nginx配置重载失败"
        return 1
    fi
    echo "✅ nginx配置重载成功"
...
cleanup_temp_files() {
    ...
    # 清理其他可能的临时文件
    find "$(dirname "$NGINX_CONF_PATH")" -name "nginx_temp_*" -mtime +1 -delete 2>/dev/null || true
}
...
check_deployment_status() {
    # 检查配置文件语法
    if "$NGINX_BIN_PATH" -t -c "$NGINX_CONF_PATH" -p "$(dirname "$NGINX_BIN_PATH")" -e "$NGINX_LOGS_PATH/error.log" -g "pid $NGINX_LOGS_PATH/nginx.pid;" > /dev/null 2>&1; then
        return 0
    else
        return 1
    fi
...
validate_params() {
    ...
    # 检查根路径是否存在（Windows路径兼容）
    if [ ! -d "$ROOT_PATH" ]; then
        echo "❌ 根路径不存在: $ROOT_PATH"
        echo "💡 请检查路径是否正确，Windows路径示例："
        echo "   - C:/website/app/dist"
        echo "   - ./dist"
        echo "   - ../website/dist"
        exit 1
    fi
...
check_windows_environment() {
    ...
    if [ -f "$NGINX_BIN_PATH" ]; then
        ...
        echo "❌ nginx未安装或路径不正确"
        echo "💡 请检查路径或修改脚本中的NGINX_BIN_PATH变量"
        exit 1
    fi
    ...
    if [ -f "$NGINX_CONF_PATH" ]; then
        ...
        # 检查配置文件权限
        if [ -r "$NGINX_CONF_PATH" ] && [ -w "$NGINX_CONF_PATH" ]; then
            echo "✅ nginx配置文件可读写"
        else
...
show_usage() {
    ...
    echo ""
    echo "快速示例:"
    echo "  $0 \"localhost\" \"80\" \"C:/website/app/dist\" \"/\""
    echo "  $0 \"test.funshion.tv\" \"443\" \"./dist\" \"/\" \"C:/ssl/cert.pem\" \"C:/ssl/key.key\""
    echo ""
